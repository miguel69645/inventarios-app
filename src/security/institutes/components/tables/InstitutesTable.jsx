...
const InstitutesTable = () => {
  ...
  const insituteDataArr = useSelector(
    (state) => state.institutesReducer.institutesDataArr
  );

  const updateInstitutes = (newData) => {
    dispatch(SET_ID_INSTITUTES({ institutesDataArr: newData }));

    updateInstitutes(InstitutesData[0]._id);

    const handleRowClick = (row) => {
      setSelectedInstituteId(row._id);
      updateInstitutes(row._id);
    };
